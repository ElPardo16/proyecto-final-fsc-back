...
export default function index({ data, people }) {
   ...
   const filterData = _ => {
      ...
      switch (orderRef.current.value) {
         case "1":
            filterPeople = [...filterPeople.sort((a, b) => a.fName?.localeCompare(b.fName))]
            break
         case "2":
            filterPeople = [...filterPeople.sort((a, b) => b.fName?.localeCompare(a.fName))]
            break
      }
      if (posRef.current.value !== "0") {
         filterPeople = [...filterPeople.filter(item => item.position === posRef.current.value)]
      }
      if (search.value !== "") {
         if (searchOptRef.current.value === "name") {
            filterPeople = [...filterPeople.filter(item => {
               const name = `${item.fName} ${item.sName} ${item.fName} ${item.fName}`
               return name.toLowerCase().includes(search.value.toLowerCase())
            })]
         } else {
            filterPeople = [...filterPeople.filter(item => item[searchOptRef.current.value].includes(search.value))]
         }
      }
         ...
         <div className="filters">
            ...
            <div className="options">
               ...
               <select onChange={filterData} ref={posRef} name="" id="">
                  <option value="0">Por Defecto</option>
                  {position.map(item => <option key={item} value={item}>{item}</option>)}
               </select>
            </div>
            ...
            <div className="search">
               <input onChange={filterData} type={inputType} name="search" id="search" placeholder='Buscar' />
